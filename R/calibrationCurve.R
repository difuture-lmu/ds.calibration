...
#' @param nprivacy [integer(1L)] Number of values per bin to ensure privacy (default is 5).
#' @return Brier score and number of values used as weights for later aggregation.
#' @author Daniel S.
#' @export
calibrationCurve = function (truth_name, prob_name, nbins = 10L, nprivacy = 5L)
{
  ...
  checkmate::assertCount(nprivacy, na.ok = FALSE, positive = TRUE, null.ok = FALSE)
  ...
  ## Calculate curve only if each bin has at least five or more values to ensure privacy:
  if (any(tb[tb > 0] < nprivacy)) stop("More than ", nprivacy, " ovservations per bin are required to ensure privacy!")

  df_tmp = data.frame(truth, prob)
  out    = aggregate(df_tmp, by = list(bin = bins), FUN = "mean")
...
#' @author Daniel S.
#' @export
dsCalibrationCurve = function (connections, truth_name, pred_name, nbins = 10L, nprivacy = 5L)
{
  checkmate::assertCharacter(truth_name, len = 1L, null.ok = FALSE, any.missing = FALSE)
  ...
  call = paste0("calibrationCurve(\"", truth_name, "\", \"", pred_name, "\", ", nbins,", ", nprivacy, ")")
  cq = NULL
  ...
  aggregated = data.frame(bin = bins, lower = individuals[[1]]$lower, upper = individuals[[1]]$upper, truth = truth, prob = prob)
...
#' @param individuals [logical(1L)] Logical value indicating whether the individual calibration curves should be plotted or not (default is `TRUE`).
...
plotCalibrationCurve = function (cc, individuals = TRUE)
{
  ...
  if (individuals) {
    gg = gg +
      geom_point(data = tmp, aes(x = prob, y = truth, color = Server), alpha = 0.5) +
      geom_line(data = tmp, aes(x = prob, y = truth, color = Server), alpha = 0.5)
  }
  gg = gg +
    geom_point(data = cc$aggregated, aes(x = prob, y = truth)) +
    geom_line(data = cc$aggregated, aes(x = prob, y = truth))

  gg = gg +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "dark red") +
    xlab("Predicted") +
    ylab("True frequency") +
    theme_minimal() +
